...
export async function createExcelComunizados<K extends keyof QueryResultMap>(
  data: WithoutID<QueryResultMap[K]>[],
) {
  const workbook = new Workbook()
  const worksheet = workbook.addWorksheet('COMUNIZADOS')
  ...
  titleCell.value = `RELATÓRIO DE PRODUTOS COMUNIZADOS`
  ...
  headerRow.eachCell((cell, idx) => {
    ...
    cell.font = {
      ...
      bold: true,
      size: 8,
      color: idx === 4 ? { argb: 'FFFFFF' } : { argb: '000000' },
    }
  })
